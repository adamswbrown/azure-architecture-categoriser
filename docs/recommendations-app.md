...
This application accepts **two types of input files**:

1. **App Cat Context Files** (Java/.NET applications) - Generated by [Dr. Migrate](https://drmigrate.com) during the [AppCat assessment process](https://learn.drmigrate.com/docs/using-dr-migrate/migration-planning/appcat-github-integration/)

2. **Dr. Migrate Data Exports** (ALL applications) - Generated from Dr. Migrate AI Advisor using the provided LLM prompt. This enables recommendations for applications without Java/.NET components.

Both formats are auto-detected on upload. See [Dr. Migrate Integration](drmigrate-integration.md) for details on using the LLM prompt.

If you don't have a context file, you can use the [sample files](../examples/context_files/) for testing.
...
The application accepts two file formats that are automatically detected on upload.

### Format 1: App Cat Context Files (Java/.NET)

Generated by Dr. Migrate during the AppCat assessment process:
...
### Format 2: Dr. Migrate Data Exports (ALL Applications)

Generated from Dr. Migrate AI Advisor. This format enables recommendations for applications without Java/.NET components:
...
```json
{
  "application_overview": {
    ...
    "other_tech_stack_components": "Python, Django, PostgreSQL"
  },
  "server_overviews": [
    {
      "machine": "APP-WEB-01",
  ...
  "installed_applications": [
    {
      "machine": "APP-WEB-01",
      "key_software": "Python 3.9"
    }
  ]
}
```

Dr. Migrate format is automatically converted to context format on upload. See [Dr. Migrate Integration](drmigrate-integration.md) for the full LLM prompt to generate this data.

### Required Fields (App Cat Format)
...
### Required Fields (Dr. Migrate Format)

| Field | Description |
|-------|-------------|
| `application_overview` | Object with application metadata |
| `application_overview.application` | Application name |
...
- Is valid JSON
- Is an array with at least one object (App Cat format) OR an object with `application_overview` (Dr. Migrate format)
- Contains `app_overview` with an `application` name (App Cat) OR `application_overview.application` (Dr. Migrate)
- Includes `detected_technology_running` and `server_details` fields (App Cat only)

If validation fails, the app provides:
- Specific error messages and suggestions
- An expandable section with the LLM prompt for Dr. Migrate AI Advisor
- Instructions for generating correct data format
